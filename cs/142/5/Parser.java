    ...
    private void procedureFormalParams(SymbolTable parameters, SymbolTable table) {
        if (nextIsType ()){
            do {
                ...
                String name = ident();
                SymbolTable.Symbol.SimplyTyped.Variable variable = new SymbolTable.Symbol.SimplyTyped.Variable (type, codeGen.getNextLocalLocation (4), reference);
        ...
        if (sym==Scanner.CONST||sym==Scanner.TYPE||sym==Scanner.VAR){
            while ((sym==Scanner.CONST||sym==Scanner.TYPE||sym==Scanner.VAR)){
                if (accept(Scanner.CONST)){
                    ...
                    SymbolTable.TypeDesignator type = newType (table);
                    expect(Scanner.SEMI_COLON);
                    for (String name : names) bind (table, name, new SymbolTable.Symbol.SimplyTyped.Variable (type, codeGen.getNextGlobalLocation (4), false));
                }
            }
        }
    }
